...
/* GUID used by SVENTX decoder on host tool */
static const sven_guid_t sven_logcat_guid = {
    0x7953a961, 0xeb7a, 0x4d76, { 0x82, 0xd7, 0xe3, 0x1c, 0x40, 0xf1, 0x27, 0xb3 } };

static bool g_outSventx = false;
#endif
...
#ifdef USES_SVENTX
        if (g_outSventx) {
            /* Write the message to NPK in the following format:
             * <priority><tag>\0x00<message>
             */
            int len = 0;
            char tmpBuf[1024];
            tmpBuf[len++] = entry.priority;
            len += snprintf(tmpBuf + len, sizeof(tmpBuf) - len, "%s", entry.tag);
            tmpBuf[len++] = 0;
            len += snprintf(tmpBuf + len, sizeof(tmpBuf) - len, "%s", entry.message);
...
#ifdef USES_SVENTX
        ret = getopt(argc, argv, "cdt:T:gG:sQf:r:n:v:b:BSpP:x");
#else
...
#define  CONSOLE_OPTION "androidboot.console="
            ...
            case 'x':
                ...
                SVEN_SET_HANDLE_GUID_UNIT(svenHandle, sven_logcat_guid, 0);
                break;
#endif
    ...
    if (!logger_list) {
        fprintf(stderr, "Unable to alloc logger list\n");
        exit(EXIT_FAILURE);
    }
