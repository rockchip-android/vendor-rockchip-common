...
void test_do_copy_tail(char *src, int limit, int expect) {
    ...
    dest = (char*)malloc(strlen(src)+11);
    if (dest) {
        sprintf(dest, "rm -f %s_copy", src);
        system(dest);
        free(dest);
    }
}
...
void test_generate_crashlog_dir(int mode, char *unique, int other_folders) {
    char temp[256];
    int count = other_folders;

    system("rm -f res/logs/currentcrashlog");
    system("rm -rf res/logs/crashlog0_*");
    while(count--) {
        snprintf(temp, 256, "mkdir res/logs/crashlog0_temp_%d", count);
        system(temp);
    }

    count = dir_contains("res/logs/", "crashlog0_", 0);

    if (count != other_folders) {
        printf("%s failed on creating new paths; created %d\n", __FUNCTION__, count);
        return;
    }

    char *path = generate_crashlog_dir(mode, unique);
    if (!path || !directory_exists(path)) {
        printf("%s failed on creating new crash directory; mode: %d unique: %s\n",
                __FUNCTION__, mode, unique);
        return;
    }

    if (dir_contains("res/logs/", "crashlog0_", 0) != 1) {
        printf("%s failed on removing old paths; matching paths present: %d\n", __FUNCTION__, count);
        return;
    }

    printf("%s new path created: %s: succeeded\n", __FUNCTION__, path);
    rmfr(path);
    free(path);
}
...
int main(int __attribute__((unused)) argc, char __attribute__((unused)) **argv) {
    ...
    */
    ...
    test_generate_crashlog_dir(MODE_CRASH, "A5A5A5", 2);
    ...
    /* Cleanup */
    system("rm -f res/file_to_append");
    system("rm -f res/logs/currentaplogslog");
    system("rm -f res/logs/currentbzlog");
    system("rm -f res/logs/currentcrashlog");
    system("rm -f res/logs/currentstatslog");
