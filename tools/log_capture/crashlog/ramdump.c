...
static int crashlog_check_ramdump(const char * reason)
{
    ...
    if( !file_exists(LM_DUMP_FILE) )
        ...
        snprintf(destination, sizeof(destination), "%s/%s_%s.bin",
                 dir, SAVED_LM_BUFFER_NAME, dateshort);
    ...
    if ( !file_exists(LBR_DUMP_FILE) )
       ...
       snprintf(destination, sizeof(destination), "%s/%s_%s.txt",
                dir, SAVED_LBR_BUFFER_NAME, dateshort);
...
#define CMD_REBOOT "reboot"
    ...
    run_command(CMD_SET_RESET_TO_GLOBAL, 30);
...
int do_ramdump_checks(int test) {
    ...
    /* Raise REBOOT event*/
    key = generate_event_id(SYS_REBOOT, RAMCONSOLE);
    raise_event(key, SYS_REBOOT, RAMCONSOLE, NULL, NULL);
