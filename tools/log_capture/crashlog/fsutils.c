...
static const char *flush_aplog_cmd = "/system/bin/logcat -b all -v threadtime -d -f " APLOG_FILE_0;
static const char *flush_aplog_boot_cmd = "/system/bin/logcat -b all -v threadtime -d -f "
        APLOG_FILE_BOOT;
...
void activate_logcatext_collection() {
    flush_aplog_cmd = LOGCATEXT_PATH " -b all -v threadtime -d -f " APLOG_FILE_0;
    flush_aplog_boot_cmd = LOGCATEXT_PATH " -b all -v threadtime -d -f " APLOG_FILE_BOOT;
}
...
void copy_bplogs(const char *extra, char *dir, int limit, int instance) {
    ...
    snprintf(dst, PATHMAX, "%s/%s%s%s", dir, strrchr(prop,'/')+1, extra, BPLOG_FILE_0_EXT);
    if(stat(src, &info) == 0) {
        do_copy_tail(src, dst, limit);
        if(info.st_size < 1*MB) {
            snprintf(src, PATHMAX, "%s%s", prop, BPLOG_FILE_1_EXT);
            snprintf(dst, PATHMAX, "%s/%s%s%s", dir, strrchr(prop,'/')+1, extra, BPLOG_FILE_1_EXT);
            do_copy_tail(src, dst, limit);
        }
    }
}
...
static void move_logfile(char *mode, const char *logfile0, const char *logfile1,
    ...
    if (stat(logfile0, &info) == 0) {
        snprintf(destination, sizeof(destination), "%s/%s_%s_%s%s",
                 dir, strrchr(logfile0, '/') + 1, mode, timestamp,
                 extension);
        do_copy_tail((char *)logfile0, destination, limit);
        if (info.st_size < 1 * MB) {
            snprintf(destination, sizeof(destination), "%s/%s_%s_%s%s",
                     dir, strrchr(logfile1, '/') + 1, mode,
                     timestamp, extension);
...
void do_log_copy(char *mode, char *dir, const char* timestamp, int type) {
    ...
    case KDUMP_TYPE:
        ...
        move_logfile(mode, logfile0, logfile1, timestamp, extension, limit, dir);
#ifndef CONFIG_APLOG
        remove(APLOG_FILE_0);
#endif
        break;
...
static void copy_bplogs_old(char *mode, const char* timestamp, 
                            char *dir, int limit, int instance) {
    char destination[PATHMAX], bp_extra[PATHMAX], *logfile0, *logfile1, *extension;
...
void do_bplog_copy(char *mode, char *dir, const char* timestamp, int type, int instance) {
    char destination[PATHMAX], bp_extra[PATHMAX], *logfile0, *logfile1, *extension;
    int limit = MAXFILESIZE;
    int mdm_instance;
    ...
    case BPLOG_STATS_TYPE:
        ...
        case COLLECT_BPLOG_CRASHING_MODEM:
            copy_bplogs(bp_extra, dir, limit, instance);
            break;
        case COLLECT_BPLOG_CRASHING_ALL:
            mdm_instance = get_modem_count();
            while (mdm_instance--)
                copy_bplogs(bp_extra, dir, limit, mdm_instance);
            break;
        default:
            break;
        }
    ...
    case BPLOG_TYPE_OLD:
        ...
        switch (cfg_collection_mode_modem()) {
        case COLLECT_BPLOG_CRASHING_MODEM:
            copy_bplogs_old(mode, timestamp, dir, limit, instance);
            break;
        case COLLECT_BPLOG_CRASHING_ALL:
            mdm_instance = get_modem_count();
            while (mdm_instance--)
                copy_bplogs_old(mode, timestamp, dir, limit, mdm_instance);
            break;
        default:
            break;
        }
