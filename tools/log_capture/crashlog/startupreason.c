...
static int compute_uefi_startupreason(enum reset_sources rs, enum reset_types rt,
                                      ...
                                      enum shutdown_sources ss __attribute__((__unused__)),
                                      char *startupreason) {
...
static struct {
    ...
    {"PMIC_WATCHDOG", "HWWDT_RESET"},
    {"EC_WATCHDOG", "HWWDT_RESET"},
    {"PLATFORM_SPECIFIC", "HWWDT_RESET"}
};
 ...
 *
 * If startup reason contains HWWDT or SWWDT and watchdog contains
 * UNHANDLED, generate a WDT crash event. Copy aplogs, pstore console
 * and FW data files in the event.
 *
 * @param reason - string containing the translated startup reason
 * @param watchdog - watchdog type
 */
int crashlog_check_startupreason(char *reason, char *watchdog) {
    ...
    LOGV("%s: reason:%s watchdog:%s\n", __FUNCTION__, reason, watchdog);
    if ( !( strstr(reason, "WDT_") ) || strstr(reason, "FAKE") || !( strstr(watchdog, "UNHANDLED") ) ) {
        /* Nothing to do */
        return 0;
    }

    if (is_event_type_fake(watchdog)) {
        raise_infoerror(ERROREVENT, watchdog);
        return 0;
    }

    key = generate_event_id(CRASHEVENT, watchdog);
    dir = generate_crashlog_dir(MODE_CRASH, key);
    if (!dir) {
        LOGE("%s: generate_crashlog_dir failed\n", __FUNCTION__);
        raise_event(key, CRASHEVENT, watchdog, NULL, NULL);
        LOGE("%-8s%-22s%-20s%s\n", CRASHEVENT, key, get_current_time_long(0), "WDT");
        free(key);
        return -1;
    }

    raise_event_wdt(key, CRASHEVENT, watchdog, reason, dir);
    LOGE("%-8s%-22s%-20s%s %s\n", CRASHEVENT, key, get_current_time_long(0), "WDT", dir);
